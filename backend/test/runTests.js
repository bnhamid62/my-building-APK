    ...
    server.listen(0, async () => {
        ...
        try {
            ...
            await test('Auth: Login with valid credentials & return JWT', async () => {
                ...
                if (!data.token || data.user.role !== 'OWNER_SYNDIC')
                    throw new Error('Invalid auth response payload');
                syndic1Token = data.token;
            });
            await test('Auth: Login Syndic 2 (apt20)', async () => {
                const res = await fetch(`${baseUrl}/api/v1/auth/login`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: 'apt20', password: 'amarati123' })
                });
                const data = await res.json();
                syndic2Token = data.token;
            });
            await test('Auth: Login Standard Owner (apt2)', async () => {
                const res = await fetch(`${baseUrl}/api/v1/auth/login`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (data.user.role !== 'OWNER')
                    throw new Error('Role must be OWNER');
            ...
            await test('Structure: Authoritative 40 apartments on 10 levels (RDC + Floors 1..9)', async () => {
                ...
                if (apts.length !== 40)
                    throw new Error(`Expected 40 apartments, got ${apts.length}`);
                // Check RDC (apts 1..4)
                for (let i = 1; i <= 4; i++) {
                    const apt = apts.find(a => a.number === i);
                    if (!apt || apt.floor_label !== 'RDC' || apt.floor !== 0)
                        throw new Error(`Apt ${i} should be on RDC`);
                }
                // Check Floor 9 (apts 37..40)
                for (let i = 37; i <= 40; i++) {
                    const apt = apts.find(a => a.number === i);
                    if (!apt || apt.floor_label !== '9' || apt.floor !== 9)
                        throw new Error(`Apt ${i} should be on Floor 9`);
                }
            });
            ...
            await test('Dual-Syndic: Syndic 1 creates project -> PENDING_APPROVAL', async () => {
                ...
                if (p.status !== 'PENDING_APPROVAL')
                    throw new Error('Status must be PENDING_APPROVAL');
                if (p.contribution_per_apt !== 5000)
                    throw new Error('Contribution must be 200000 / 40 = 5000');
                testProjectId = p.id;
            });
            ...
            await test('Dual-Syndic: Syndic 2 approves project -> APPROVED (200)', async () => {
                ...
                if (p.status !== 'APPROVED')
                    throw new Error('Status must be APPROVED');
            });
            ...
            await test('Payment: Syndic 1 records payment directly -> LOCKED with sequential TX ID', async () => {
                ...
                if (tx.status !== 'LOCKED')
                    throw new Error('Payment must be immediately LOCKED');
                if (!tx.tx_id.startsWith('TX-2026-'))
            ...
            await test('Payment: Idempotent duplicate submission returns existing TX without duplicate credit', async () => {
                ...
                const tx = await res.json();
                if (tx.tx_id !== paymentTxId)
                    throw new Error('Idempotency failure: new transaction was generated');
            });
            ...
            await test('Correction: Syndic 2 requests correction linked to original TX -> PENDING_APPROVAL', async () => {
                ...
                if (corr.status !== 'PENDING_APPROVAL')
                    throw new Error('Status must be PENDING_APPROVAL');
                if (corr.original_tx_id !== paymentTxId)
                    throw new Error('Original TX link missing');
            ...
            await test('Correction: Approved by Syndic 1 -> LOCKED', async () => {
                ...
                if (res.status !== 200)
                    throw new Error(`Expected 200, got ${res.status}`);
                const corr = await res.json();
                if (corr.status !== 'LOCKED')
                    throw new Error('Status must be LOCKED');
            });
            ...
            await test('Sync Engine: Batch sync creates maintenance and promotes payment', async () => {
                ...
                if (res.status !== 200)
                    throw new Error(`Expected 200, got ${res.status}`);
                const data = await res.json();
                if (data.synced.length !== 2)
                    throw new Error('Expected 2 synced items');
                if (data.synced[1].status !== 'LOCKED')
                    throw new Error('Payment must be server LOCKED');
            });
